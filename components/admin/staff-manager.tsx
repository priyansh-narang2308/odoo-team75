...
export function StaffManager() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      if (editMember) {
        ...
        setStaff((prev) =>
          prev.map((s) => (s.id === editMember.id ? data.data : s)),
        );
      } else {
  ...
  const toggleActive = async (m: StaffMember) => {
    ...
    const data = await res.json();
    if (data.ok)
      setStaff((prev) => prev.map((s) => (s.id === m.id ? data.data : s)));
  };
        ...
        {(["ADMIN", "CASHIER", "KITCHEN"] as const).map((role) => {
          ...
          return (
            <div
              key={role}
              className="card"
              style={{ padding: "18px", border: `1px solid ${cfg.color}22` }}
            >
              <div
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            >
              {editMember ? "Edit Staff" : "Add Staff Member"}
            </h3>
            {error && (
              <div
                style={{
